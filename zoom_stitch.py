...
def video_generator(assets_actor, assets_doctor, zoom_bg, output_dir, emp_name=None, sub_dir=None, font_path=None, text_font_path=None, bg_music=None, vid_lang=None):
    try:
        driving_flag = 'right'  # Start with 'right' for zoom stitching
        if not os.path.exists(output_dir):
            os.makedirs(output_dir)

        actor_files = sorted([f for f in os.listdir(assets_actor) if f.endswith('.mp4')])
        
        # Build a dictionary mapping index to doctor video filename
        # Doctor files are named video_XX.mp4 where XX is the original index
        doctor_files_dict = {}
        for f in os.listdir(assets_doctor):
            if f.endswith('.mp4'):
                # Extract index from filename like video_02.mp4
                idx = int(f.split('_')[1].split('.')[0])
                doctor_files_dict[idx] = f
        ...
        for i in range(len(actor_files)):
            ...
            # Check if there's a doctor video for this index - if yes, apply zoom stitch
            if i in doctor_files_dict:
                doctor_video = os.path.join(assets_doctor, doctor_files_dict[i])
                ...
                # No doctor video for this index, just copy actor video
...
def remote_video_stitching(input_obj): 
    try:
        ...
        final_wrapped_video_path = os.path.join(curr_dir.name, 'final_wrapped.mp4')
        final_intro_local = os.path.join(curr_dir.name, 'final_intro.mp4')
        final_outro_local = os.path.join(curr_dir.name, 'final_outro.mp4')
        ...
        download_from_cloud(input_obj["background_image"], zoom_bg, folder=False, data_cloud="gcp")
        download_from_cloud(input_obj["font_path"], font_file_path, folder=False, data_cloud="gcp")
        download_from_cloud(input_obj["text_font_path"], text_font_file_path, folder=False, data_cloud="gcp")
        download_from_cloud(input_obj["subtitle_files"], subtitles_dir, folder=True, data_cloud="gcp")
        for i, video_gcp_url in enumerate(input_obj["assets_actor_paths"]):
            ...
            download_from_cloud(video_gcp_url, video_path, folder=False, data_cloud="gcp")

        for i, video_gcp_url in enumerate(input_obj["assets_doctor_paths"]):
            if video_gcp_url is None or str(video_gcp_url).lower() == "none" or str(video_gcp_url).lower() == "null":
                logger.info("Skipping null doctor video at index {}".format(i))
                continue
        ...
        # Audio replacement for placeholder videos
        audio_overlays = input_obj.get('audio_overlays', [])
        if audio_overlays:
            logger.info("Processing {} audio overlays...".format(len(audio_overlays)))
            for overlay in audio_overlays:
                segment_index = overlay['segment_index']
                audio_path = overlay['audio_path']
                trim_video = overlay.get('trim_video_to_audio', False)
                
                logger.info("Processing audio overlay for segment {}".format(segment_index))
                
                # Download audio file
                audio_file = os.path.join(curr_dir.name, 'audio_{}.mp3'.format(segment_index))
                download_from_cloud(audio_path, audio_file, folder=False, data_cloud="gcp")
                
                # Get corresponding actor video
                actor_video_file = 'video_{:02}.mp4'.format(segment_index)
                actor_video_path = os.path.join(local_assets_actor, actor_video_file)
                
                # Replace audio and optionally trim
                output_path = actor_video_path.replace('.mp4', '_audio_replaced.mp4')
                if trim_video:
                    # Get audio duration
                    audio_duration = get_duration(audio_file)
                    # Trim video to audio duration and replace audio
                    replace_audio_cmd = "ffmpeg -i {} -i {} -t {} -map 0:v -map 1:a -c:v libx264 -c:a aac -shortest {}".format(
                        actor_video_path, audio_file, audio_duration, output_path)
                else:
                    # Just replace audio without trimming
                    replace_audio_cmd = "ffmpeg -i {} -i {} -map 0:v -map 1:a -c:v copy -c:a aac -shortest {}".format(
                        actor_video_path, audio_file, output_path)
                
                subprocess.run(replace_audio_cmd, shell=True, check=True)
                logger.info("Audio replaced for segment {}".format(segment_index))
                
                # Replace original with audio-replaced version
                os.remove(actor_video_path)
                os.rename(output_path, actor_video_path)
        
        # Nodding video trimming
        nodding_segments = input_obj.get('nodding_segments', [])
        if nodding_segments:
            logger.info("Processing {} nodding segments...".format(len(nodding_segments)))
            for nodding in nodding_segments:
                nodding_index = nodding['nodding_index']
                match_duration_of = nodding['match_duration_of']
                
                logger.info("Processing nodding segment {} to match duration of segment {}".format(
                    nodding_index, match_duration_of))
                
                # Get doctor video duration
                doctor_video_file = 'video_{:02}.mp4'.format(match_duration_of)
                doctor_video_path = os.path.join(local_assets_doctor, doctor_video_file)
                doctor_duration = get_duration(doctor_video_path)
                
                # Trim nodding video
                nodding_video_file = 'video_{:02}.mp4'.format(nodding_index)
                nodding_video_path = os.path.join(local_assets_actor, nodding_video_file)
                output_path = nodding_video_path.replace('.mp4', '_trimmed.mp4')
                trim_cmd = "ffmpeg -i {} -t {} -c copy {}".format(
                    nodding_video_path, doctor_duration, output_path)
                subprocess.run(trim_cmd, shell=True, check=True)
                logger.info("Nodding video trimmed for segment {} to {} seconds".format(
                    nodding_index, doctor_duration))
                
                # Replace original with trimmed version
                os.remove(nodding_video_path)
                os.rename(output_path, nodding_video_path)
        ...
        # Optional final intro/outro wrapping
        final_intro_path = input_obj.get("final_intro_path")
        final_outro_path = input_obj.get("final_outro_path")
        final_target = final_local_video_path

        if final_intro_path and final_outro_path:
            logger.info("Wrapping with final intro/outro")
            download_from_cloud(final_intro_path, final_intro_local, folder=False, data_cloud="gcp")
            download_from_cloud(final_outro_path, final_outro_local, folder=False, data_cloud="gcp")

            concat_list = os.path.join(curr_dir.name, 'concat_list.txt')
            with open(concat_list, 'w', encoding='utf-8') as f:
                f.write("file '{}'\n".format(final_intro_local))
                f.write("file '{}'\n".format(final_local_video_path))
                f.write("file '{}'\n".format(final_outro_local))

            concat_cmd = "ffmpeg -hide_banner -loglevel error -f concat -safe 0 -i {} -c copy {}".format(concat_list, final_wrapped_video_path)
            subprocess.run(concat_cmd, shell=True, check=True)
            final_target = final_wrapped_video_path
            logger.info("Final intro/outro wrapped")

        upload_to_cloud(final_target, input_obj["final_upload_path"], cloud="gcp")
        end = time.time()
...
def gcp_to_https(gcp_uri):
    if not gcp_uri.startswith("gs://"):
        raise ValueError("Invalid GCP URI. Must start with gs://")
    gcp_path = gcp_uri[5:]
    https_url = "https://storage.googleapis.com/{}".format(gcp_path)
    return https_url
...
def handle_response(request_data, success=True):
    ...
    final_upload_path = request_data.get("final_upload_path", "")
    if final_upload_path.startswith("gs://"):
        http_final_upload_path = gcp_to_https(final_upload_path)
    elif final_upload_path.startswith("s3://"):
        http_final_upload_path = s3_to_https(final_upload_path)
    else:
        http_final_upload_path = final_upload_path
    if webhook_url:
        response_data = {
